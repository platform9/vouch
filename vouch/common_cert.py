...
# this file is shared by both deccaxon and vouch
...
def get_latest_ca_cert(format='cert'):
    ...
    for secret in secrets.items:
        ...
        if match:
            ...
            LOG.info(f'{secret.metadata.name}: version {version}')
...
def wait_key_from_secret(v1, secret_name, namespace_name, key):
    ...
    start = int(time.time())
    while True:
        LOG.info(f'checking if new CA cert "{secret_name}" is ready')
        secret = None
        try:
            secret = v1.read_namespaced_secret(secret_name, namespace_name)
        except Exception as e:
            if 'Reason: Not Found' in str(e):
                LOG.info(f'...secret "{secret_name}" not found')
            else:
                LOG.info(f'failed to read secret "{secret_name}" in namespace "{namespace_name}": {str(e)}')
        if secret and secret.data:
            if key in secret.data:
                ...
                if len(data_b64) > 10:
                    ...
                    LOG.info(f'key "{key}" was found but it is much too short')
            else:
                LOG.info(f'secret does not yet have "{key}" present')
        now = int(time.time())
        if now - start > TIMEOUT:
            raise(f'failed to obtain new CA certificate after {TIMEOUT} seconds')
        time.sleep(3)

def create_self_signed_issuer(issuer_name):
    ...
    for issuer in issuers.items:
        ...
        if issuer['metadata']['name'] == issuer_name:
            exists = True

    if exists:
        LOG.info(f'issuer "{issuer_name} already exists, no need to create')
        return
    ...
    LOG.info(f'self-signed issuer "{issuer_name}" successfully created')

def create_cert_from_issuer(name, issuer):
    ...
    new_cert = {
            ...
            "spec": {
                ...
                "subject": {
                    "organizationalUnits": [ 'Laurentian' ],
                    "organizations": [ 'Rutabaga Services' ],
                },
    ...
    return secret_name, cert_name, cert

def create_new_ca(name):

    create_self_signed_issuer("self-signed-issuer")
    secret_name, cert_name, cert = create_cert_from_issuer(name, "self-signed-issuer")
    ...
    if exists:
        raise Exception(f'issuer {issuer_name} already exists')

    # create an issuer linked to the above newly generated self-signed cert
...
def sign_csr(cert_name, csr, private_key, issuer=None, ip_sans=None, alt_names=None, ttl=None):
    ...
    try:
        ...
        if 'already exists' in str(e.body):
            LOG.info(f'secret {secret_name} already exists, patching it...')
            api_response = v1.patch_namespaced_secret(secret_name, namespace, pk_secret)
        else:
            raise
    ...
    if not issuer:
        ...
        signer_name = 'issuers.cert-manager.io/' + namespace + '.' + issuer
    ...
    for i in range(5):
        ...
        try:
            ...
            LOG.info(f"kubectl command failed with return code {e.returncode}")
            LOG.info("kubectl stderr:", e.stderr)
        except FileNotFoundError:
            raise Exception("unable to approve certificate: kubectl is not in the PATH")
        except Exception as e:
            last_exception = e
            LOG.info('error attempting to approve "%s": %s', (cert_name, e))
        time.sleep(1+i)

    if not success:
        raise Exception(f'unable to approve certificate: {last_exception}: {last_stderr}')
