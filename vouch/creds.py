...
def get_keystone_creds(request, user):
        ...
        """
        LOG.info(f'fetching credentials for user "{user}"')

        whitelist = os.environ["CREDS_API_USER_WHITELIST"]
        whitelist = whitelist.split(",")
        whitelist = [w.strip() for w in whitelist]

        if user not in whitelist:
            raise SanicException(f'user {user} is not whitelisted for this API')
